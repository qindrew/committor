...
class FSpectralABF(GriddedSamplingMethod):
    """
...
def _spectral_abf(method, snapshot, helpers):
    ...
    def initialize():
        ...
        return SpectralABFState(xi, bias, hist, Fsum, force, Wp, Wp_, fun, 1,restr,proj,Frestr)

    def update(state, data):
        # During the intial stage use ABF
        nstep = state.nstep
        in_fitting_regime = (nstep > fit_threshold) & (nstep < fit_threshold_upper)
        ...
        # Calculate geometric restraint force
        e_f, proj = ext_force(data)
        bias = np.reshape(-Jxi.T @ force, state.bias.shape)-np.reshape(
            e_f, state.bias.shape
        )
        # Calculate restraint contribution to force
        restr = solve_pos_def(Jxi @ Jxi.T, Jxi @ e_f.flatten())
        # Store restraint contribution in grid
        Frestr = state.Frestr.at[I_xi].add(restr)
        return SpectralABFState(xi, bias, hist, Fsum, force, Wp, state.Wp, fun, state.nstep + 1,restr,proj,Frestr)

    return snapshot, initialize, generalize(update, helpers, jit_compile=False)


def build_free_energy_fitter(method: FSpectralABF, fit):
    """
...
@dispatch
def build_force_estimator(method: FSpectralABF):
    """
...
@dispatch
def analyze(result: Result[FSpectralABF]):
    """
    Parameters
    ----------
    ...
    dict:
        ...
        fes_corrected_fn: Callable[[JaxArray], JaxArray]
            Function that allows to interpolate the corrected free energy in the CV domain defined
            by the grid.

        free_energy_corrected: JaxArray
            Free energy corrected at each bin in the CV grid.
    ...
    def build_fes_fn(fun):
        return lambda x: evaluate(fun, x)

    def first_or_all(seq):
        return seq[0] if len(seq) == 1 else seq
    def fit_forces_rst(state):
        shape = (*state.Frestr.shape[:-1], 1)
        force = state.Frestr / np.maximum(state.hist.reshape(shape), 1)
        return fit(force)
    def fit_forces_corr(state):
        shape = (*state.Frestr.shape[:-1], 1)
        force = (state.Frestr+state.Fsum) / np.maximum(state.hist.reshape(shape), 1)
        return fit(force)
    ...
    for s in states:
        fes_corr = build_fes_fn(fit_forces_corr(s))
        fes_rst = build_fes_fn(fit_forces_rst(s))
        ...
        fes_fns_corr.append(fes_corr)
        fes_fns_rst.append(fes_rst)
        free_energies_corr.append(fes_corr(mesh))
        free_energies_rst.append(fes_rst(mesh))
        mean_restraints.append(average_forces(s.hist, s.Frestr))
        mean_forces_corr.append(average_forces(s.hist, s.Frestr+s.Fsum))
    ...
    ana_result = dict(
        ...
        mean_restraint=first_or_all(mean_restraints),
        force_corrected=first_or_all(mean_forces_corr),
        fes_corrected_fn=first_or_all(fes_fns_corr),
        fes_rst_fn=first_or_all(fes_fns_rst),
        free_energy_corrected=first_or_all(free_energies_corr),
        free_energy_restraint=first_or_all(free_energies_rst),
    )
...
#            f.write(str(proj) + "\n")
    ...
    def __call__(self, snapshot, state, timestep):
        ...
        if self.counter >= self.log_period and self.counter % self.log_period == 0:
            self.save_work(state.xi,) # state.perp, state.restr)
